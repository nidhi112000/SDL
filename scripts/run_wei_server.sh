...
tmux send-keys -t $session:$window 'cd ' $folder C-m
tmux send-keys -t $session:$window 'python3 -m wei.server --workcell ~/workspace/BIO_workcell/bio_workcell.yaml' C-m
...
tmux send-keys -t $session:$window 'python3 -m wei.engine --workcell ~/workspace/BIO_workcell/bio_workcell.yaml' C-m
